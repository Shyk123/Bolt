...
//  while (j < textNew[i].toCharArray().size) {
    ...
    fun deletWords(text: String, engWord: String, rusWord: String): String {
        ...
        var slovaVstroke: Array<String> = arrayOf()
        ...
        var textNew2: CharArray = charArrayOf()
        ...
        while (i < textNew.count()) {
            ...
            j2 = 0
            n = 1
            textNew2 = textNew[i].toCharArray() // строка для работы

            while ( textNew2[j] != '/' ) {      // take eng. word
                textPriom[j2] =  textNew2[j] // больше не вылетает, вылетает дальше
                j++
                j2++
            }

            slovaVstroke[0]  = textPriom.concatToString() // превращаем в string
            textPriom = charArrayOf() // обнуляем

            j2 = 0

            while ( textNew2[j] != '>' ) {
                if ( textNew2[j] != ',' ) {
                    textPriom[j2] = textNew2[j]
                    j++
                    j2++
                }
                else {
                    j++
                    slovaVstroke[n]  = textPriom.concatToString()
                    textPriom = charArrayOf() // обнуляем
                    j2 = 0
                    n++
                }
            ...
            textPriom = slovaVstroke[0].toCharArray()
            j2 = textPriom.size
// --------------------- основная проверка
            g = 0
            poz = 0
            ...
            if ((j2 == engWord.toCharArray().size) || (j2 + 1 == engWord.toCharArray().size) || (j2 - 1 == engWord.toCharArray().size)) {
                ...
                if ( g < 2 ) {	// ----- попадаем сюда если в слове менее двух ошибок
                    ...
                    while ( j3<n+1 ) {
                        textPriom = charArrayOf() 	// обнуляем
                        textPriom = slovaVstroke[j].toCharArray()
                        j2 = textPriom.size
                        g = 0
